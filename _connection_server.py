...
N_PORT = constants.SERVER_PORT
N_PORT_REC = constants.SERVER_PORT_REC
...
def listener():
	...
	with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
		...
		while True:
			...
			conn, addr = s.accept()
			print("Server-listener connected")
			...
			with conn:
				while True:
					...
					with open("__conn_server__.dat", "wb") as f:
						f.write(data)
...
def fetch():
	...
	while tmp is None:
		time.sleep(2)
		try:
			with open("__conn_server__.dat", "rb") as f:
				tmp = f.read() or None
			with open("__conn_server__.dat", "wb") as f:
				pass
		except Exception:
